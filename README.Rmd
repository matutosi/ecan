...
# ind_val() is a permutation test, so fix the seed to keep the
# rendered p-values (and thus the diff of README.md) reproducible.
set.seed(1)
```
...
version on CRAN**, so the two do not match.
It already includes features that are not released yet,
such as `twinspan()` and its helpers.
Install it if you need them.
...
the modified TWINSPAN of Roleček et al. (2009).
It needs no compiler, but it is not a port of Hill's original FORTRAN
program: see `?twinspan` for the known differences.
...
The modified TWINSPAN divides the most heterogeneous group first,
so that the number of groups can be chosen directly.
