...
#' 
#' @return  A dataframe with columns: stand, layer, species, cover, st_group and sp_group
#' 
#' @examples
#' n <- 300
#' height_max <- 20
...
gen_example_layer <- function(n = 300, 
  ...
  if(use_layer) stand <- dplyr::left_join(stand, layer)
  ...
  if(use_layer){
    comp <- 
      comp %>%
      bind_cols(tibble::tibble(layer = sample(ly_list, n, replace = TRUE)))
  }
  comp <- 
    {if(use_layer){
      dplyr::group_by(comp, stand, layer, species)
    } else {
      dplyr::group_by(comp, stand, species)
    }} %>%
    dplyr::summarise(cover = sum(cover), .groups = "drop") %>%
    dplyr::ungroup()
  df <- 
    {if(use_layer){
      dplyr::arrange(comp, stand, layer, species)
    } else {
      dplyr::arrange(comp, stand, species)
    }} %>%
